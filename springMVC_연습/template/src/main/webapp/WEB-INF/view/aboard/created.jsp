...
<form name="createdForm" method="post">
	...
	<div class="form-group">
	  ...
	  <textarea class="form-control" id="contents" name="contents" rows="5">${dto.contents }</textarea>
	</div>
	...
	<div style="text-align: center;">
		...
		<button class="btn btn-sm btn-dark" type="button" onclick="javascript:location.href='<%=cp%>/aboard/list';">취소</button>
	</div>
</form>
</div>
...
function checkInput(){
	...
	var mode = "${mode}";
	if(mode=='created'){
		form.action = "<%=cp%>/aboard/created";	
	} else if(mode=='update') {
		form.action = "<%=cp%>/aboard/update";	
	} else {
		form.action = "<%=cp%>/aboard/answer"
	}
